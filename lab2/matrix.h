...
public:
    explicit TwoDimensionMatrix();
    TwoDimensionMatrix(TwoDimensionMatrix const &twoDimensionMatrix);
    explicit TwoDimensionMatrix(const MatrixElement matrix[size][size]);
    [[nodiscard]] MatrixElement get(int, int) const;
...
std::ostream &operator<<(std::ostream &output, TwoDimensionMatrix const& matrix);
